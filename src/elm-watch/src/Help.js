...
import { ELM_WATCH_EXIT_ON_STDIN_END, ELM_WATCH_OPEN_EDITOR, NO_COLOR, } from "./Env";
import { bold, dim, join } from "./Helpers";
const elmWatchJson = bold("elm-watch.json");
export function render(loggerConfig) {
    // Not trimming on purpose.
    const symbolLegend = loggerConfig.fancy
        ? `
${bold("Symbol legend:")}

${join(Object.values(EMOJI).map(({ emoji, description }) => {
            const indent = "    ";
