...
export function init(cwd, logger, args) {
    if (isNonEmptyArray(args)) {
        logger.writeToStderrMakesALotOfSenseHere(`${bold("elm-watch init")} takes no arguments.`);
        return 1;
    }
    ...
    if (fs.existsSync(elmWatchJsonPath.theElmWatchJsonPath.absolutePath)) {
        logger.writeToStderrMakesALotOfSenseHere(`${elmWatchJson} already exists in the current directory!`);
        return 1;
    }
    ...
    try {
        fs.writeFileSync(elmWatchJsonPath.theElmWatchJsonPath.absolutePath, example);
    }
    catch (unknownError) {
        const error = toError(unknownError);
        logger.writeToStderrMakesALotOfSenseHere(`Failed to write ${elmWatchJson}:\n\n${error.message}`);
        return 1;
    }
    logger.write(`
