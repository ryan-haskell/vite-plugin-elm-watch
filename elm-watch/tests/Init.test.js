...
import { assertExitCode, clean, logDebug, MemoryWriteStream, readFile, rm, SilentReadStream, stringSnapshotSerializer, testExceptWindows, } from "./Helpers";
const FIXTURES_DIR = path.join(__dirname, "fixtures", "init");
async function initSuccessHelper(fixture) {
...
async function initFailHelper(fixture, ...args) {
    const dir = path.join(FIXTURES_DIR, fixture);
    const stdout = new MemoryWriteStream();
    const stderr = new MemoryWriteStream();
