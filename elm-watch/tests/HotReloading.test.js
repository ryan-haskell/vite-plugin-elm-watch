...
import { rimraf, stringSnapshotSerializer, TerminalColorReadStream, testExceptWindows, touch, wait, waitOneFrame, } from "./Helpers";
import { assertCompilationMode, assertDebugDisabled, assertDebugger, cleanupAfterEachTest, click, closeOverlay, expandUi, FIXTURES_DIR, getOverlay, hideErrors, runHotReload, showErrors, switchCompilationMode, } from "./HotHelpers";
...
describe("hot reloading", () => {
    ...
    test("Html", async () => {
        ...
        const { renders } = await go(({ idle, div }) => {
            switch (idle) {
                case 1:
                    assertDebugDisabled();
                    assertInit(div);
                    replace((content) => content.replace("hot reload", "simple text change"));
                    return "KeepGoing";
                case 2:
                    assertHotReload(div);
                    replace((content) => content.replace("simple text change", "hot reload"));
                    return "KeepGoing";
                case 3:
      ...
      ▼ 🚀 ✅ 13:10:05 HtmlMain
    `);
        function assertInit(div) {
            expect(div.outerHTML).toMatchInlineSnapshot(`<div><h1 class="probe">hot reload</h1></div>`);
            probe = div.querySelector(".probe");
            expect(probe?.outerHTML).toMatchInlineSnapshot(`<h1 class="probe">hot reload</h1>`);
        }
        function assertHotReload(div) {
            expect(div.outerHTML).toMatchInlineSnapshot(`<div><h1 class="probe">simple text change</h1></div>`);
            expect(div.querySelector(".probe") === probe).toMatchInlineSnapshot(`true`);
        }
    });
    test.each([
        ["Sandbox", "standard"],
    ...
    ])("DOM and Msg change: %s / %s", async (programType, compilationMode) => {
        const { replace, go } = runHotReload({
            ...
            switch (idle) {
                case 1:
                    ...
                    replace((content) => content
                        ...
                        .replace("onClick OriginalButtonClicked", "onClick NewButtonClicked"));
                    return "KeepGoing";
                default:
        ...
        async function assertInit(main) {
          ...
          `);
            ...
            expect(probe?.outerHTML).toMatchInlineSnapshot(`<h1 class="probe">Before hot reload</h1>`);
            click(main, "button");
            await waitOneFrame();
            expect(main.outerHTML).toMatchInlineSnapshot(`
            <main><h1 class="probe">Before hot reload</h1><button>Button</button><pre>
        ...
        async function assertHotReload(main) {
            expect(main.outerHTML).toMatchInlineSnapshot(`
          ...
          `);
            expect(main.querySelector(".probe") === probe).toMatchInlineSnapshot(`true`);
            click(main, "button");
            await waitOneFrame();
            expect(main.outerHTML).toMatchInlineSnapshot(`
            <main><h1 class="probe">After hot reload</h1><button>Button</button><pre>
    ...
    test.each(["standard", "debug", "optimize"])("Application URL messages change: %s", async (compilationMode) => {
        const { replace, go } = runHotReload({
            name: "Application",
        ...
        await go(async ({ idle, main }) => {
            switch (idle) {
                case 1:
                    ...
                    replace((content) => content
                        .replace("Before hot reload", "After hot reload")
                        .replace("onUrlRequest = OriginalUrlRequested", "onUrlRequest = NewUrlRequested")
                        .replace("onUrlChange = OriginalUrlChanged", "onUrlChange = NewUrlChanged"));
                    return "KeepGoing";
                default:
                    await assertHotReload(main);
                    return "Stop";
    ...
    test.each([
        ...
        ["Worker", "standard"],
        ["Worker", "optimize"],
    ])("Port change: %s / %s", async (programType, compilationMode) => {
        ...
        await go(async ({ idle }) => {
            switch (idle) {
                case 1:
                    await assertInit();
                    replace((content) => content.replace("fromJs OriginalFromJs", "fromJs NewFromJs"));
                    return "KeepGoing";
                default:
        ...
        async function assertInit() {
            sendToElm(1);
            await waitOneFrame();
            expect(lastValueFromElm.value).toMatchInlineSnapshot(`Before hot reload: [1]`);
        }
        async function assertHotReload() {
            sendToElm(2);
            await waitOneFrame();
            expect(lastValueFromElm.value).toMatchInlineSnapshot(`Before: [1]. After hot reload: [2]`);
        }
    });
    test.each([
        ["Element", "standard"],
        ...
        ["Application", "debug"],
        ["Application", "optimize"],
    ])("Add subscription: %s / %s", async (programType, compilationMode) => {
        const { replace, go } = runHotReload({
        ...
        const { browserConsole } = await go(async ({ idle, main }) => {
            switch (idle) {
                case 1:
                    await assertInit(main);
                    replace((content) => content.replace(/-- /g, "").replace("Sub.none", ""));
                    return "KeepGoing";
                default:
        ...
        function assertBrowserConsole() {
            expect(browserConsole).toMatchInlineSnapshot(``);
        }
        function assertBrowserConsoleOptimize() {
            expect(browserConsole).toMatchInlineSnapshot(`
        ...
        async function assertReloadForOptimize(main) {
            expect(main.outerHTML).toMatchInlineSnapshot(`<main>0</main>`);
            main.click();
            await waitOneFrame();
    ...
    describe("All program types", () => {
            ...
            const { replace, go } = runHotReload({
                ...
                init: () => {
                    ...
                    if (subscribe === undefined) {
                        throw new Error("WorkerProgram app.ports.output.subscribe should exist.");
                    }
                    subscribe((value) => {
      ...
      `);
            function assert1(body) {
                expect(removeDebugger(body)).toMatchInlineSnapshot(`<body><p>ApplicationProgram (1)</p></body>`);
                expect(removeDebugger(container)).toMatchInlineSnapshot(`<div><p>HtmlProgram (1)</p><p>SandboxProgram (1)</p><p>ElementProgram (1)</p><p>WorkerProgram (1)</p></div>`);
            }
            function assert2(body) {
                expect(removeDebugger(body)).toMatchInlineSnapshot(`<body><p>ApplicationProgram (2)</p></body>`);
                expect(removeDebugger(container)).toMatchInlineSnapshot(`<div><p>HtmlProgram (2)</p><p>SandboxProgram (2)</p><p>ElementProgram (2)</p><p>WorkerProgram (2)</p></div>`);
            }
            function removeDebugger(element) {
    ...
    test("remove input file", async () => {
        ...
        ✅ ⧙13:10:05⧘ Compilation finished in ⧙123 ms⧘.
      `);
        async function assert1(div) {
            expect(div.outerHTML).toMatchInlineSnapshot(`<div><div><h1>hot reload</h1><button>Button</button><pre>0</pre></div></div>`);
            click(div, "button");
            await waitOneFrame();
            expect(div.outerHTML).toMatchInlineSnapshot(`<div><div><h1>hot reload</h1><button>Button</button><pre>1</pre></div></div>`);
        }
        function assert2(div) {
            expect(div.outerHTML).toMatchInlineSnapshot(`<div><div><h1>simple text change</h1><button>Button</button><pre>1</pre></div></div>`);
        }
    });
    ...
    test.each(["standard", "debug", "optimize"])("Add Msg: %s", async (compilationMode) => {
        const { replace, go } = runHotReload({
            name: "AddMsg",
            programType: "Element",
            compilationMode,
    ...
    test("Init new field", async () => {
      ...
      `);
        ...
        function assert2(div) {
            expect(div.outerHTML).toMatchInlineSnapshot(`<div>field1 with newField</div>`);
        }
    });
    test("Init change cmd / Add port used in init", async () => {
        ...
        const { browserConsole } = await go(({ idle }) => {
            switch (idle) {
                case 1:
                    assert1();
                    replace((content) => content.replace("module", "port module").replace(/-- /g, ""));
                    return "KeepGoing";
                default:
                    assert2();
                    return "Stop";
    ...
    `);
        function assert1(main) {
            expect(main.outerHTML).toMatchInlineSnapshot(`<main>0</main>`);
        }
        function assert2(main) {
    ...
    test("View fails after hot reload", async () => {
        ...
        expect(browserConsole.replace(/(\n\s*at _String_toUpper).*(\n\s*at.+)*/, "$1")).toMatchInlineSnapshot(`
    ...
    test("Init with non-cancelable Task", async () => {
        ...
        async function assert1(div) {
            ...
            expect(button.outerHTML).toMatchInlineSnapshot(`<button id="id">Count: 0</button>`);
            button.click();
            await waitOneFrame();
            expect(button.outerHTML).toMatchInlineSnapshot(`<button id="id">Count: 1</button>`);
        }
        function assert2(div) {
            const button = div.querySelector("button");
            if (button === null) {
            ...
            expect(document.activeElement).toBe(button);
            expect(button.outerHTML).toMatchInlineSnapshot(`<button id="id">Hot count: 1</button>`);
        }
    });
    describe("Init with cancelable Task", () => {
        ...
        test("Init with cancelable Task", async () => {
            ...
            async function assert1(div) {
                ...
                expect(button.outerHTML).toMatchInlineSnapshot(`<button>Count: 0</button>`);
                button.click();
                await waitOneFrame();
                expect(button.outerHTML).toMatchInlineSnapshot(`<button>Count: 1</button>`);
            }
            function assert2(div) {
                const button = div.querySelector("button");
                if (button === null) {
                ...
                expect(button.outerHTML).toMatchInlineSnapshot(`<button>Hot count: 1</button>`);
            }
        });
    });
    describe("Html.Lazy", () => {
        ...
        test("Html.Lazy", async () => {
            ...
            await go(async ({ idle, main }) => {
                switch (idle) {
                    case 1:
                        await assert1(main);
                        replace((content) => content.replace("Is divisible by", "HOT RELOADED $&"));
                        return "KeepGoing";
                    default:
                        await assert2(main);
                        return "Stop";
        ...
        `);
            async function assert1(main) {
                expect(main.outerHTML).toMatchInlineSnapshot(`<main><p>Number: 0</p><p>Is divisible by 4? Yes.</p></main>`);
                expect(mockConsoleLog.mock.calls.length).toMatchInlineSnapshot(`1`);
                main.click();
                await waitOneFrame();
                expect(main.outerHTML).toMatchInlineSnapshot(`<main><p>Number: 1</p><p>Is divisible by 4? No.</p></main>`);
                expect(mockConsoleLog.mock.calls.length).toMatchInlineSnapshot(`2`);
                main.click();
                await waitOneFrame();
                expect(main.outerHTML).toMatchInlineSnapshot(`<main><p>Number: 2</p><p>Is divisible by 4? No.</p></main>`);
                expect(mockConsoleLog.mock.calls.length).toMatchInlineSnapshot(`2`);
            }
            async function assert2(main) {
                expect(main.outerHTML).toMatchInlineSnapshot(`<main><p>Number: 2</p><p>HOT RELOADED Is divisible by 4? No.</p></main>`);
                expect(mockConsoleLog.mock.calls.length).toMatchInlineSnapshot(`3`);
                main.click();
                await waitOneFrame();
                expect(main.outerHTML).toMatchInlineSnapshot(`<main><p>Number: 3</p><p>HOT RELOADED Is divisible by 4? No.</p></main>`);
                expect(mockConsoleLog.mock.calls.length).toMatchInlineSnapshot(`3`);
                main.click();
                await waitOneFrame();
                expect(main.outerHTML).toMatchInlineSnapshot(`<main><p>Number: 4</p><p>HOT RELOADED Is divisible by 4? Yes.</p></main>`);
    ...
    test("Html.map", async () => {
        ...
        async function assert1(main) {
            expect(main.outerHTML).toMatchInlineSnapshot(`<main><button>Count: 0</button></main>`);
            click(main, "button");
            await waitOneFrame();
            expect(main.outerHTML).toMatchInlineSnapshot(`<main><button>Count: 2</button></main>`);
        }
        async function assert2(main) {
            expect(main.outerHTML).toMatchInlineSnapshot(`<main><button>HOT RELOADED Count: 2</button></main>`);
            click(main, "button");
            await waitOneFrame();
            expect(main.outerHTML).toMatchInlineSnapshot(`<main><button>HOT RELOADED Count: 4</button></main>`);
        }
        async function assert3(main) {
            expect(main.outerHTML).toMatchInlineSnapshot(`<main><button>HOT RELOADED Count: 4</button></main>`);
            click(main, "button");
            await waitOneFrame();
            expect(main.outerHTML).toMatchInlineSnapshot(`<main><button>HOT RELOADED Count: 7</button></main>`);
        }
        async function assert4(main) {
            expect(main.outerHTML).toMatchInlineSnapshot(`<main><button>HOT RELOADED Count: 7</button></main>`);
            click(main, "button");
            await waitOneFrame();
            expect(main.outerHTML).toMatchInlineSnapshot(`<main><button>HOT RELOADED Count: 4</button></main>`);
        }
        async function assert5(main) {
            expect(main.outerHTML).toMatchInlineSnapshot(`<main><button>HOT RELOADED Count: 4</button></main>`);
            click(main, "button");
            await waitOneFrame();
            expect(main.outerHTML).toMatchInlineSnapshot(`<main><button>HOT RELOADED Count: -1</button></main>`);
        }
    });
    ...
    test("Changed record fields in optimize with postprocess", async () => {
      ...
      `);
        ...
        function assertHotReload(div) {
            expect(div.outerHTML).toMatchInlineSnapshot(`<div>Text and new text</div>`);
        }
    });
    ...
    describe("error overlay", () => {
        ...
        const replaceHelper = (name, f) => {
            fs.writeFileSync(path.join(src, name), f(fs.readFileSync(path.join(src, name), "utf-8")));
        };
        const joinOverlays = (overlays) => overlays.join(`\n${"=".repeat(80)}\n`);
        beforeEach(async () => {
            await rimraf(src);
            fs.mkdirSync(src);
        ...
        test("multiple targets", async () => {
            ...
            const { renders } = await go(({ idle }) => {
                switch (idle) {
                    ...
                    case 2:
                        replaceHelper("AppHelpers.elm", (content) => content.replace(/"/g, "'"));
                        return "KeepGoing";
                    case 3:
                        ...
                        return "KeepGoing";
                    case 4:
                        replaceHelper("AppOtherHelpers.elm", (content) => content.replace("=", ":="));
                        return "KeepGoing";
                    case 5:
                        expandUi("AppOther");
                        showErrors("AppOther");
                        ...
                        return "KeepGoing";
                    case 6:
                        replaceHelper("Shared.elm", (content) => content.replace("text", "textTypo"));
                        return "KeepGoing";
                    case 7:
