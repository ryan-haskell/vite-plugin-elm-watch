...
import { mapNonEmptyArray } from "../src/NonEmptyArray";
...
function walkImportsHelper(fixture, inputFiles, sourceDirectories, { resolveSymlinks = false } = {}) {
    const dir = absolutePathFromString(FIXTURES_DIR, fixture);
    const inputPaths = mapNonEmptyArray(inputFiles, (inputFile) => {
    ...
    const result = walkImports(mapNonEmptyArray(sourceDirectories, (sourceDirectory) => ({
        tag: "SourceDirectory",
...
function printRelatedElmFilePaths(relatedElmFilePaths) {
    return Array.from(relatedElmFilePaths, (filePath) => filePath.startsWith(FIXTURES_DIR.absolutePath)
...
describe("WalkImports", () => {
    ...
    test("multiple source directories", () => {
        expect(walkImportsHelper("multiple-source-directories", ["app/Main.elm"], ["app", "body-parts", "units"])).toMatchInlineSnapshot(`
    ...
    test("multiple inputs", () => {
        expect(walkImportsHelper("multiple-inputs", ["App.elm", "Admin.elm"], ["."])).toMatchInlineSnapshot(`
    ...
    test("duplicate imports", () => {
        expect(walkImportsHelper("duplicate-imports", ["DuplicateImports.elm"], ["."])).toMatchInlineSnapshot(`
    ...
    test("ambiguous source directories", () => {
        ...
        walkImportsHelper("anywhere", ["Main.elm"], [".", "src"])).toMatchInlineSnapshot(`
      /anywhere/Main.elm
      /anywhere/src/Main.elm
    `);
    });
    test("ambiguous source directories – more complex", () => {
        expect(walkImportsHelper("anywhere", ["src/App/Main.elm"], ["lib", "src", "src/App"])).toMatchInlineSnapshot(`
    ...
    test("ambiguous source directories – multiple inputs", () => {
        expect(walkImportsHelper("anywhere", ["src/App/Main.elm", "src/App/Other.elm"], ["lib", "src", "src/App"])).toMatchInlineSnapshot(`
      /anywhere/src/App/Main.elm
      /anywhere/lib/App/Main.elm
      /anywhere/src/App/App/Main.elm
      /anywhere/lib/Main.elm
    ...
    testExceptWindows("symlinks", () => {
        ...
        expect(walkImportsHelper("symlinks", ["MainSymlink.elm"], ["."], {
    ...
    describe("cycles", () => {
        test("import self", () => {
            expect(walkImportsHelper("cycles", ["ImportSelf.elm"], ["."])).toMatchInlineSnapshot(`/cycles/ImportSelf.elm`);
        });
        ...
        test("import entrypoint indirect", () => {
            expect(walkImportsHelper("cycles", ["ImportEntryPointIndirect.elm"], ["."])).toMatchInlineSnapshot(`
