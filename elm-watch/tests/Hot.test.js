...
import { __ELM_WATCH_ELM_TIMEOUT_MS, __ELM_WATCH_EXIT_ON_WORKER_LIMIT, __ELM_WATCH_OPEN_EDITOR_TIMEOUT_MS, __ELM_WATCH_WORKER_LIMIT_TIMEOUT_MS, ELM_WATCH_EXIT_ON_STDIN_END, ELM_WATCH_OPEN_EDITOR, NO_COLOR, } from "../src/Env";
import { printTimeline } from "../src/Hot";
import { IS_WINDOWS } from "../src/IsWindows";
import { clean, CtrlCReadStream, httpGet, rimraf, rm, rmSymlink, SilentReadStream, stringSnapshotSerializer, TEST_ENV, testExceptWindows, touch, wait, } from "./Helpers";
import { assertDebugger, cleanupAfterEachTest, clickFirstErrorLocation, collapseUi, expandUi, failInit, FIXTURES_DIR, getOverlay, moveUi, run, showErrors, switchCompilationMode, } from "./HotHelpers";
...
describe("hot", () => {
    ...
    test("connect with elm.json error", async () => {
        ...
        fs.copyFileSync(path.join(dir, "elm.template.json"), path.join(dir, "elm.json"));
    ...
    describe("Parse WebSocket connect request url errors", () => {
            ...
            class TestWebSocket extends WebSocket {
                constructor(url) {
                    if (typeof url === "string") {
                        throw new Error("TestWebSocket expects the url to be a URL object, not a string!");
                    }
                    f(url);
                    super(url);
        ...
        ✅ ⧙13:10:05⧘ Everything up to date.
      `);
            expect(renders.replace(/elmCompiledTimestamp=\d+/, "elmCompiledTimestamp=1644064438938")).toMatchInlineSnapshot(`
        ▼ 🔌 13:10:05 BadUrl
        ================================================================================
        ▼ ⏳ 13:10:05 BadUrl
        ================================================================================
      ...
      `);
        ...
        test("change target name", async () => {
            ...
            const elmWatchJsonString = fs.readFileSync(elmWatchJsonTemplatePath, "utf8");
            ...
            const { terminal, renders } = await run({
                ...
                onIdle: ({ idle }) => {
                    switch (idle) {
                        case 1:
                            fs.writeFileSync(elmWatchJsonPath, elmWatchJsonString.replace("Main", "Renamed"));
                            return "KeepGoing";
                        case 2:
                            expandUi();
            ...
            onIdle: async ({ idle, div }) => {
                switch (idle) {
                    case 1:
                        assert1(div);
                        touch(roguePath);
                        fs.writeFileSync(elmWatchJsonPath, elmWatchJsonString.slice(0, -10));
                        await wait(100);
                        fs.writeFileSync(elmWatchJsonPath, elmWatchJsonString.replace(/"postprocess":.*/, ""));
                        return "KeepGoing";
                    case 2:
                        assert2(div);
                    ...
                    case 3:
                        ...
                        return "KeepGoing";
                    default:
                        throw new Error("Expected elm-watch to exit due to no elm-watch.json!");
                }
            },
        });
    ...
    test("changes to elm-watch-node JS file", async () => {
        ...
        const postprocessTemplatePath = path.join(FIXTURES_DIR, fixture, "postprocess.template.js");
            ...
            onIdle: ({ idle, div }) => {
                switch (idle) {
                    case 1:
                        ...
                        return "KeepGoing";
                    case 2:
                        fs.writeFileSync(postprocessPath, postprocessString.replace("toUpperCase", "toLowerCase"));
                        return "KeepGoing";
                    case 3:
                        assert2(div);
    ...
    testExceptWindows("typecheck-only should not break because of duplicate inputs", async () => {
        const { terminal, renders } = await run({
    ...
    `);
        function assert1(div) {
            expect(div.outerHTML).toMatchInlineSnapshot(`<div>postprocess content before</div>`);
        }
        function assert2(div) {
            expect(div.outerHTML).toMatchInlineSnapshot(`<div>postprocess content after</div>`);
        }
    });
    ...
    describe("click error location", () => {
        ...
        const runFailClickErrorLocation = async (env) => {
            const { renders } = await run({
                fixture,
                args: [],
                scripts: ["Main.js"],
      ...
      `);
        ...
        test("unknown command", async () => {
            ...
            const cleanedRenders = renders
                ...
                // Windows
                .replace("'nope' is not recognized as an internal or external command,\r\noperable program or batch file.", replacement)
      ...
      `);
        ...
        test("successful execution", async () => {
            const outputFile = path.join(FIXTURES_DIR, fixture, "click-error-location.txt");
            rm(outputFile);
        ...
        ▼ 🚨 13:10:05 Main
      `);
            expect(clean(fs.readFileSync(outputFile, "utf-8"))).toMatchInlineSnapshot(`/Users/you/project/tests/fixtures/hot/persisted-open-error-overlay/src/Main.elm:10:31`);
        });
    });
    test("persisted debug mode for Html", async () => {
        ...
        const { terminal, renders } = await run({
            ...
            onIdle: ({ body }) => {
                // No debugger.
                expect(body.outerHTML).toMatchInlineSnapshot(`<body><div>Html</div></body>`);
                expandUi();
                return "Stop";
            },
        });
    ...
    test("WebSocket server HTTP HTML page", async () => {
        ...
        const dir = path.join(FIXTURES_DIR, fixture);
        const elmWatchJsonPath = path.join(dir, "elm-watch.json");
        const elmWatchJson = JSON.parse(fs.readFileSync(elmWatchJsonPath, "utf8"));
        const port = Decode.fields((field) => field("port", Decode.number))(elmWatchJson);
        let mainHtml = "(not set)";
        let variations = ["(not set)"];
        await run({
            fixture,
            args: ["Main"],
            scripts: ["Main.js"],
    ...
    test("reload trouble with http caching", async () => {
        ...
        const cleanedRenders = renders.replace(/compiled .+? and/, "compiled 10/9/2022, 11:36:01 AM, and");
        expect(cleanedRenders).toMatchInlineSnapshot(`
    ...
    describe("printTimeline", () => {
        function print(events, loggerConfig) {
        ...
        test("1 event", () => {
            expect(print(events.slice(0, 1))).toMatchInlineSnapshot(`⧙ℹ️ 23:59:05 Changed /One.elm⧘`);
        });
        test("2 events", () => {
