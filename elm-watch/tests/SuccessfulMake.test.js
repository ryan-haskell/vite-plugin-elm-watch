...
import { NO_COLOR } from "../src/Env";
import { assertExitCode, clean, CursorWriteStream, logDebug, MemoryWriteStream, prependPATH, rimraf, SilentReadStream, stringSnapshotSerializer, TEST_ENV, } from "./Helpers";
const FIXTURES_DIR = path.join(__dirname, "fixtures");
async function run(fixture, args, { isTTY = true, bin, env } = {}) {
    ...
    const exitCode = await elmWatchCli(args, {
        ...
        env: {
            ...
            ...TEST_ENV,
            ...env,
            PATH: bin === undefined ? process.env.PATH : prependPATH(path.join(dir, bin)),
        },
...
describe("successful make", () => {
    ...
    test("installed packages output", async () => {
        expect(await run("successful-make", ["make"], {
    ...
    test("CI", async () => {
        expect(await run("successful-make", ["make", "--optimize"], { isTTY: false })).toMatchInlineSnapshot(`
      ⏳ Dependencies
      ✅ Dependencies
