...
import { __ELM_WATCH_LOADING_MESSAGE_DELAY, __ELM_WATCH_MAX_PARALLEL, __ELM_WATCH_MOCKED_TIMINGS, __ELM_WATCH_QUERY_TERMINAL_MAX_AGE_MS, __ELM_WATCH_QUERY_TERMINAL_TIMEOUT_MS, ELM_WATCH_OPEN_EDITOR, WT_SESSION, } from "../src/Env";
import { printStdio } from "../src/Errors";
import { toError, toJsonError, } from "../src/Helpers";
import { IS_WINDOWS } from "../src/IsWindows";
toError.jestWorkaround = (arg) => arg;
toJsonError.jestWorkaround = (arg) => arg;
...
export function badElmBinEnv(dir) {
    return {
        ...process.env,
        ...TEST_ENV,
...
export function prependPATH(folder) {
    ...
    if (IS_WINDOWS && fs.existsSync(path.join(folder, "elm"))) {
        fs.writeFileSync(path.join(folder, "elm.cmd"), `@echo off\r\nnode "%~dp0\\elm" %*`);
    }
...
export function rmSymlink(symlink) {
...
export class SilentReadStream extends stream.Readable {
    constructor() {
        super(...arguments);
        this.isTTY = true;
        this.isRaw = false;
...
export class TerminalColorReadStream extends SilentReadStream {
    on(eventName, listener) {
        if (eventName === "data" && this.isRaw) {
            this.data.push(...Array.from({ length: 16 }, (_, i) => {
                const hex = i.toString(16).toUpperCase().repeat(4);
                return `\x1B]4;${i};rgb:${hex}/${hex}/${hex}${i % 2 === 0 ? "\x07" : "\x1B\\"}`;
            }), "\x1B]10;rgb:1111/2222/3333\x07", "\x1B]11;rgb:aaaa/bbbb/cccc\x1B\\");
            this._read();
        }
        return super.on(eventName, listener);
    }
}
export class CtrlCReadStream extends SilentReadStream {
    ctrlC() {
        this.data.push("\x03");
        this._read();
    }
...
function parseEscape(char, num1, num2) {
    switch (char) {
        case "A":
...
function colorAwareSlice(string, start, end = string.length) {
    let result = "";
    let index = 0;
...
export class CursorWriteStream extends stream.Writable {
    constructor() {
        super(...arguments);
        this.isTTY = true;
        this.columns = 80;
    ...
    _write(chunk, _encoding, callback) {
        const parts = removeTerminalColorRequests(chunk.toString()).split(SPLIT_REGEX);
        for (const part of parts) {
            switch (part) {
                ...
                default: {
                    ...
                    if (match !== null) {
                        const escape = parseEscape(match[3], toNumber(match[1]), toNumber(match[2]));
                        switch (escape.tag) {
                            case "ClearLineFull":
                                this.lines[this.cursor.y] = "";
                                break;
                            case "ClearLineToEnd":
                                this.lines[this.cursor.y] = colorAwareSlice(this.lines[this.cursor.y] ?? "", 0, this.cursor.x);
                                break;
                            case "ClearLineToStart":
                                this.lines[this.cursor.y] = colorAwareSlice(this.lines[this.cursor.y] ?? "", this.cursor.x);
                                break;
                            ...
                            case "ClearScreenToEnd":
                                this.lines = this.lines.map((line, index) => index < this.cursor.y
                                    ? line
                                    : index === this.cursor.y
                            ...
                            case "CursorMove": {
                                ...
                                if (cursor.x < 0 || cursor.y < 0) {
                                    callback(new Error(`Cursor out of bounds: ${JSON.stringify(this.cursor)} + ${JSON.stringify({ dx, dy })} = ${JSON.stringify(cursor)}`));
                                    return;
                                }
                                else {
                                    this.cursor = cursor;
                        ...
                        const xDiff = this.cursor.x - stringLength(line.replace(COLOR_REGEX, ""));
                        const paddedLine = xDiff > 0 ? line + " ".repeat(xDiff) : line;
                        const partLength = stringLength(part.replace(COLOR_REGEX, ""));
                        const nextLine = colorAwareSlice(paddedLine, 0, this.cursor.x) +
                            part +
    ...
    getOutput() {
        ...
        return this.lines.join("\n").replace(RegExp(`(${Object.values(EMOJI)
            .map(({ emoji }) => emoji)
            .join("|")})  `, "g"), "$1 ");
    }
    resize(columns) {
...
function removeTerminalColorRequests(string) {
...
export function clean(string) {
    ...
    return string
        ...
        .replace(/\x1B\[0?m/g, "⧘")
        .replace(/[A-Z]:\\(\S+)/g, (_match, rest) => `/${rest.replace(/\\\\/g, "/").replace(/\\/g, "/")}`)
        .replace(/(\bcd|--output=\/dev\/null) '([^'\s]+)'/g, "$1 $2")
        .replace(/'(--output=[^'\s]+)' '([^'\s]+)'/g, "$1 $2")
        .replace(/Expected .+ in JSON at position \d+|Unexpected token . in JSON at position \d+|Unexpected end of JSON input/g, "(JSON syntax error)")
        .replace(/(EISDIR: illegal operation on a directory, read) '.+/g, "$1")
        .replace(/EPERM: operation not permitted/g, "EACCES: permission denied")
        .replace(/EOF/g, "EPIPE");
}
export function assertExitCode(expectedExitCode, actualExitCode, stdout, stderr) {
...
export async function httpGet(urlString, options = {}) {
        ...
        (urlString.startsWith("https:") ? https : http)
            ...
            res.on("end", () => {
                ...
                if (res.statusCode === 200) {
                    ...
                    reject(new Error(`GET ${urlString} – expected status code 200 but got ${res.statusCode ?? "(no status code)"}:\n\n${body}`));
                }
            });
        })
            .on("error", reject);
