...
import { __ELM_WATCH_EXIT_ON_ERROR, __ELM_WATCH_MAX_PARALLEL, __ELM_WATCH_TMP_DIR, NO_COLOR, } from "../src/Env";
import * as Errors from "../src/Errors";
import { removeColor, toError } from "../src/Helpers";
import { assertExitCode, badElmBinEnv, clean, CursorWriteStream, describeExceptWindows, logDebug, MemoryWriteStream, prependPATH, rimraf, rm, rmSymlink, SilentReadStream, stringSnapshotSerializer, TEST_ENV, testExceptWindows, wait, } from "./Helpers";
const FIXTURES_DIR = path.join(__dirname, "fixtures", "errors");
async function run(fixture, args, options) {
    return runAbsolute(path.join(FIXTURES_DIR, fixture), args, options);
}
async function runAbsolute(dir, args, { env, isTTY = true, exitHotOnError = false, } = {}) {
...
async function runWithBadElmBin(fixture, { postprocess = false, exitHotOnError = false, } = {}) {
    const dir = path.join(FIXTURES_DIR, "valid");
    const BUILD = path.join(dir, "build");
    await rimraf(BUILD);
    return runAbsolute(postprocess ? path.join(dir, "postprocess") : dir, [exitHotOnError ? "hot" : "make", "app"], {
        env: badElmBinEnv(path.join(dir, "bad-bin", fixture)),
        exitHotOnError,
    });
}
async function runWithBadElmBinAndWrittenError(fixture) {
    const dir = path.join(FIXTURES_DIR, "valid");
    const readErrorPaths = () => fs
        .readdirSync(dir)
        .filter((item) => /^elm-watch-ElmMakeJsonParseError-[^.]+\.txt$/.test(item))
    ...
    try {
        ...
        throw new Error(`Expected ${errorPath} to exist.\n\n${error.message}\n\n${output}`);
    }
    return { output, writtenError: clean(writtenError) };
}
function printError(errorTemplate) {
    return clean(Errors.toTerminalString(errorTemplate, 80, false));
}
expect.addSnapshotSerializer(stringSnapshotSerializer);
describe("errors", () => {
    test("unknown command", async () => {
        expect(await run("wherever", ["nope"])).toMatchInlineSnapshot(`Unknown command: nope`);
    });
    ...
    describe("elm-watch.json decode errors", () => {
      ...
      `);
        ...
        describe("bad target name", () => {
            test("starts with dash", async () => {
                expect(await run("elm-watch-json-bad-target-name/starts-with-dash", ["make"])).toMatchInlineSnapshot(`
          ⧙-- INVALID elm-watch.json FORMAT -----------------------------------------------⧘
        ...
        `);
            ...
            test("starts with whitespace", async () => {
                expect(await run("elm-watch-json-bad-target-name/starts-with-whitespace", [
                    "make",
                ])).toMatchInlineSnapshot(`
          ⧙-- INVALID elm-watch.json FORMAT -----------------------------------------------⧘
        ...
        `);
            ...
            test("contains newline", async () => {
                expect(await run("elm-watch-json-bad-target-name/contains-newline", ["make"])).toMatchInlineSnapshot(`
          ⧙-- INVALID elm-watch.json FORMAT -----------------------------------------------⧘
        ...
        `);
            ...
            test("ends with whitespace", async () => {
                expect(await run("elm-watch-json-bad-target-name/ends-with-whitespace", [
                    "make",
                ])).toMatchInlineSnapshot(`
          ⧙-- INVALID elm-watch.json FORMAT -----------------------------------------------⧘
        ...
        test("bad output extension", async () => {
            expect(await run("elm-watch-json-decode-error/bad-output-extension", ["make"])).toMatchInlineSnapshot(`
        ⧙-- INVALID elm-watch.json FORMAT -----------------------------------------------⧘
      ...
      `);
        ...
        test("bad output extension – just .js", async () => {
            ...
            expect(await run("elm-watch-json-decode-error/bad-output-extension-just-dot-js", ["make"])).toMatchInlineSnapshot(`
        ⧙-- INVALID elm-watch.json FORMAT -----------------------------------------------⧘
      ...
      `);
        ...
        test("bad input extension", async () => {
            expect(await run("elm-watch-json-decode-error/bad-input-extension", ["make"])).toMatchInlineSnapshot(`
        ⧙-- INVALID elm-watch.json FORMAT -----------------------------------------------⧘
      ...
      `);
        ...
        test("bad input module name", async () => {
            expect(await run("elm-watch-json-decode-error/bad-input-module-name", ["make"])).toMatchInlineSnapshot(`
        ⧙-- INVALID elm-watch.json FORMAT -----------------------------------------------⧘
    ...
    test("elm-watch.json not found and suggest JSON from args", async () => {
        expect(await runAbsolute(path.parse(__dirname).root, [
            "make",
            "src/Game.elm",
            "--output",
            "dist/game.js",
    ...
    describe("suggest JSON from args", () => {
      ...
      `);
        ...
        test("no suggesting for unknown flags", async () => {
            expect(await run("valid", ["make", "src/App.elm", "--loglevel=silent", "-f"])).toMatchInlineSnapshot(`
        ⧙-- UNEXPECTED FLAGS ------------------------------------------------------------⧘
    ...
    test("unknown targets", async () => {
        expect(await run("valid", ["make", "build/app.js", "build/adnim.js", "app.js"])).toMatchInlineSnapshot(`
    ...
    describe("elm.json errors", () => {
      ...
      `);
        ...
        test("is a folder", async () => {
            expect(await run("elm-json-is-folder", ["hot"], {
      ...
      `);
        ...
        test("bad json", async () => {
            expect(await run("elm-json-bad-json", ["hot"], {
                env: elmBinAlwaysSucceedEnv,
                exitHotOnError: true,
            })).toMatchInlineSnapshot(`
        ✅ Dependencies
      ...
      `);
        ...
        test("decode error", async () => {
            expect(await run("elm-json-decode-error", ["hot"], {
                env: elmBinAlwaysSucceedEnv,
                exitHotOnError: true,
            })).toMatchInlineSnapshot(`
        ✅ Dependencies
    ...
    describe("elm not found", () => {
        test("basic", async () => {
            expect(await run("valid", ["make"], {
                env: {
                    ...TEST_ENV,
                    PATH: [__dirname, path.join(__dirname, "some", "bin")].join(path.delimiter),
                },
            })).toMatchInlineSnapshot(`
        🚨 Dependencies
      ...
      `);
        ...
        const printPATHWindows = (env) => clean(Errors.printPATH(env, true)(80, (piece) => piece.text));
        test("Windows basic", () => {
            expect(printPATHWindows({
                Path: [__dirname, path.join(__dirname, "some", "bin")].join(path.delimiter),
            })).toMatchInlineSnapshot(`
          This is what the Path environment variable looks like:
        ...
        test("Windows no PATH-like", () => {
            expect(printPATHWindows({})).toMatchInlineSnapshot(`I can't find any program, because I can't find any PATH-like environment variables!`);
        });
    ...
    describe("elm install errors", () => {
        test("dummy file creation error", async () => {
            ...
            expect(await run(fixture, ["make", "app"], {
                env: {
                    ...process.env,
                    ...TEST_ENV,
    ...
    describe("elm make json errors", () => {
        test("syntax error", async () => {
            const { output, writtenError } = await runWithBadElmBinAndWrittenError("json-syntax-error");
            expect(output).toMatchInlineSnapshot(`
        🚨 app
      ...
      `);
        ...
        test("decode error", async () => {
            const { output, writtenError } = await runWithBadElmBinAndWrittenError("json-decode-error");
            expect(output).toMatchInlineSnapshot(`
        🚨 app
    ...
    describe("elm compilation errors", () => {
      ...
      `);
        ...
        test("--optimize with Debug.log", async () => {
            expect(await run("compilation-errors", ["make", "DebugLog", "--optimize"])).toMatchInlineSnapshot(`
        ✅ Dependencies
        🚨 DebugLog
      ...
      `);
        ...
        test("not enough bytes error", async () => {
            ...
            const result = spawn.sync("elm", ["make", "--output=/dev/null", "src/Main.elm"], {
                cwd: dir,
                encoding: "utf8",
            });
            ...
            if (result.status !== 0) {
                throw new Error(`elm make failed with status ${result.status ?? "null"}.\n\n${result.stdout}\n\n${result.stderr}`);
            }
    ...
    describe("postprocess errors", () => {
      ...
      `);
        ...
        test("exit 2 + stderr + debug", async () => {
            expect(await run("postprocess/variants/exit-2-stderr", ["make", "--debug"])).toMatchInlineSnapshot(`
        ✅ Dependencies
        🚨 main
      ...
      `);
        ...
        test("exit 3 + no output + optimize", async () => {
            expect(await run("postprocess/variants/exit-3-no-output", [
                "make",
                "--optimize",
            ])).toMatchInlineSnapshot(`
        ✅ Dependencies
      ...
      `);
        ...
        test("exit 4 + both stdout and stderr", async () => {
            expect(await run("postprocess/variants/exit-4-both-stdout-and-stderr", [
                "make",
            ])).toMatchInlineSnapshot(`
        ✅ Dependencies
        🚨 main
      ...
      `);
        ...
        test("forgot to read stdin", async () => {
            ...
            const dir = path.join(FIXTURES_DIR, fixture);
            const elmWatchJson = fs.readFileSync(path.join(dir, "elm-watch.template.json"), "utf8");
            ...
            // `echo` otherwise.
            const newElmWatchJson = process.platform === "linux"
    ...
    describe("elm-watch-node errors", () => {
      ...
      `);
        ...
        test("throw non-error at import, with console.log", async () => {
            expect(await run("postprocess/variants/throw-non-error-at-import", ["make"])).toMatchInlineSnapshot(`
        ✅ Dependencies
        🚨 main
      ...
      `);
        ...
        test("write error", async () => {
            ...
            try {
                ...
                fs.writeFileSync(elmWatchStuffJsonPath, JSON.stringify(elmWatchStuffJson), { mode: "0444" } // readonly
                );
            }
            catch {
                // Ignore write errors (readonly).
            }
            expect(await runAbsolute(dir, ["hot"], {
                env: elmBinAlwaysSucceedEnv,
                exitHotOnError: true,
            })).toMatchInlineSnapshot(`
        ✅ Dependencies
    ...
    test("import walker file system error", async () => {
        expect(await run("import-walker-file-system-error", ["hot"], {
            env: elmBinAlwaysSucceedEnv,
            exitHotOnError: true,
        })).toMatchInlineSnapshot(`
      ✅ Dependencies
    ...
    describe("port conflict", () => {
        ...
        test("for persisted port", async () => {
            expect(await run("port-conflict-for-persisted-port", ["hot"], {
                env: elmBinAlwaysSucceedEnv,
                exitHotOnError: true,
            })).toMatchInlineSnapshot(`
        ⧙-- PORT CONFLICT ---------------------------------------------------------------⧘
      ...
      `);
        ...
        test("from config", async () => {
            expect(await run("port-conflict-for-port-from-config", ["hot"], {
                env: elmBinAlwaysSucceedEnv,
                exitHotOnError: true,
            })).toMatchInlineSnapshot(`
        ⧙-- PORT CONFLICT ---------------------------------------------------------------⧘
    ...
    describe("CI", () => {
      ...
      `);
            ...
            // Postprocess error.
            expect(await run("ci", ["make", "postprocess-error"], {
                isTTY: false,
            })).toMatchInlineSnapshot(`
        ⏳ Dependencies
        ✅ Dependencies
    ...
    describe("hard to test errors", () => {
      ...
      `);
        ...
        test("otherSpawnError", () => {
            expect(printError(Errors.otherSpawnError({
                tag: "ElmJsonPath",
                theElmJsonPath: {
                    tag: "AbsolutePath",
      ...
      `);
        ...
        test("ExitReason Signal", () => {
            expect(printError(Errors.postprocessNonZeroExit({
                tag: "OutputPath",
                theOutputPath: {
                    tag: "AbsolutePath",
      ...
      `);
        ...
        test("portConflictForNoPort", () => {
            expect(printError(Errors.portConflictForNoPort(new Error("The error message")))).toMatchInlineSnapshot(`
        ⧙-- PORT CONFLICT ---------------------------------------------------------------⧘
