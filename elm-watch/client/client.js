...
import { BrowserUiPosition, } from "../src/Types";
import { decodeWebSocketToClientMessage, } from "./WebSocketMessages";
...
__ELM_WATCH.WEBSOCKET_TIMEOUT ?? (__ELM_WATCH.WEBSOCKET_TIMEOUT = 1000);
...
__ELM_WATCH.LOG_DEBUG ?? (__ELM_WATCH.LOG_DEBUG = 
// eslint-disable-next-line no-console
console.debug);
...
const ORIGINAL_COMPILATION_MODE = "%ORIGINAL_COMPILATION_MODE%";
...
// as things change.
const ORIGINAL_BROWSER_UI_POSITION = "%ORIGINAL_BROWSER_UI_POSITION%";
...
const SEND_KEY_DO_NOT_USE_ALL_THE_TIME = Symbol("This value is supposed to only be obtained via `Status`.");
...
function run() {
    let elmCompiledTimestampBeforeReload = undefined;
    try {
    ...
    const browserUiPosition = elements === undefined
    ...
    runTeaProgram({
        ...
        update: (msg, model) => {
            ...
            const reloadTrouble = model.status.tag !== updatedModel.status.tag &&
                ...
                ? [
                    ...
                    newModel.status.tag === newModel.previousStatusTag &&
                        ...
                        : {
                            ...
                            errors: newErrorOverlay === undefined ||
                                !newErrorOverlay.openErrorOverlay
                                ? new Map()
...
function getOrCreateTargetRoot() {
    ...
    if (shadowRoot === null) {
        throw new Error(`elm-watch: Cannot set up hot reload, because an element with ID ${CONTAINER_ID} exists, but \`.shadowRoot\` is null!`);
    }
    ...
    let overlayCloseButton = shadowRoot.querySelector(`.${CLASS.overlayCloseButton}`);
...
function setBrowserUiPosition(browserUiPosition, elements) {
    ...
    for (const [key, value] of Object.entries(browserUiPositionToCss(browserUiPosition))) {
    ...
    elements.shadowRoot.dispatchEvent(new CustomEvent(BROWSER_UI_MOVED_EVENT, { detail: browserUiPosition }));
}
const initMutable = (getNow, elements) => (dispatch, resolvePromise) => {
    let removeListeners = [];
    const mutable = {
        removeListeners: () => {
    ...
    mutable.webSocket.addEventListener("open", () => {
        removeListeners = [
            ...
            ...(elements === undefined
                ? []
                : [
                    addEventListener(elements.shadowRoot, BROWSER_UI_MOVED_EVENT, (event) => {
                        dispatch({
                            tag: "BrowserUiMoved",
                            browserUiPosition: Decode.fields((field) => field("detail", parseBrowseUiPositionWithFallback))(event),
                        });
                    }),
                    addEventListener(elements.shadowRoot, CLOSE_ALL_ERROR_OVERLAYS_EVENT, () => {
                        dispatch({
                            tag: "UiMsg",
                            date: getNow(),
                            msg: {
...
const init = (date, browserUiPosition, elmCompiledTimestampBeforeReload) => {
    const model = {
...
function onWebSocketToClientMessage(date, msg, model) {
    switch (msg.tag) {
        ...
        case "SuccessfullyCompiled": {
            const justChangedBrowserUiPosition = model.lastBrowserUiPositionChangeDate !== undefined &&
            ...
            return [
                {
                    ...
                    status: {
                        ...
                        sendKey: SEND_KEY_DO_NOT_USE_ALL_THE_TIME,
                        errorOverlay: {
                            errors: new Map(status.errors.map((error) => {
                                const overlayError = {
                                    title: error.title,
...
const runCmd = (getNow, elements) => (cmd, mutable, dispatch, _resolvePromise, rejectPromise) => {
    switch (cmd.tag) {
        case "Eval": {
            try {
        ...
        case "NoCmd":
            return;
        case "Reconnect":
            mutable.webSocket = initWebSocket(getNow, cmd.elmCompiledTimestamp, dispatch);
            return;
        case "Render": {
            const { model } = cmd;
            const info = {
        ...
        case "UpdateErrorOverlay":
            if (elements !== undefined) {
                updateErrorOverlay(TARGET_NAME, (msg) => {
                    dispatch({ tag: "UiMsg", date: getNow(), msg });
                }, cmd.sendKey, cmd.errors, elements.overlay, elements.overlayCloseButton);
            }
            return;
        case "UpdateGlobalStatus":
            ...
            switch (cmd.reloadStatus.tag) {
                ...
                case "ReloadRequested":
                    try {
                        window.sessionStorage.setItem(RELOAD_TARGET_NAME_KEY_PREFIX + TARGET_NAME, cmd.elmCompiledTimestamp.toString());
                    }
                    catch {
                        // Ignore failing to write to sessionStorage.
                    }
...
const ElmModule = Decode.chain(Decode.record(Decode.chain(functionToNull, Decode.multi({
    null: () => [],
    array: Decode.array(Decode.fields((field) => field("__elmWatchProgramType", ProgramType))),
...
function reloadPageIfNeeded() {
    ...
    const reasons = [];
    for (const [targetName, reloadStatus] of Object.entries(__ELM_WATCH.RELOAD_STATUSES)) {
    ...
    const [separator, reasonString] = reasons.length === 1 && first !== undefined && first[1].length === 1
        ? [" ", `${first[1].join("")}\n(target: ${first[0]})`]
        : [
            ":\n\n",
            reasons
...
function h(t, { attrs, style, localName, ...props }, ...children) {
...
function renderWebWorker(model, info) {
    ...
    return `${statusData.icon} elm-watch: ${statusData.status} ${formatTime(model.status.date)} (${info.targetName})`;
}
function render(getNow, targetRoot, dispatch, model, info, manageFocus) {
    targetRoot.replaceChildren(view((msg) => {
        dispatch({ tag: "UiMsg", date: getNow(), msg });
    }, model, info, manageFocus));
...
function getStatusClass({ statusType, statusTypeChanged, hasReceivedHotReload, uiRelatedUpdate, errorOverlayVisible, }) {
    switch (statusType) {
        case "Success":
...
function view(dispatch, passedModel, info, manageFocus) {
    ...
    return h(HTMLDivElement, { className: CLASS.container }, model.uiExpanded
        ? viewExpandedUi(model.status, statusData, info, model.browserUiPosition, dispatch)
        : undefined, h(HTMLDivElement, {
    ...
    }, icon(model.uiExpanded ? CHEVRON_UP : CHEVRON_DOWN, model.uiExpanded ? "Collapse elm-watch" : "Expand elm-watch")), compilationModeIcon(model.compilationMode), icon(statusData.icon, statusData.status, statusClass === undefined
        ? {}
        : {
        ...
        }), h(HTMLTimeElement, { dateTime: model.status.date.toISOString() }, formatTime(model.status.date)), h(HTMLSpanElement, { className: CLASS.targetName }, TARGET_NAME)));
}
function icon(emoji, alt, props) {
    return h(HTMLSpanElement, { attrs: { "aria-label": alt }, ...props }, h(HTMLSpanElement, { attrs: { "aria-hidden": "true" } }, emoji));
}
function viewExpandedUi(status, statusData, info, browserUiPosition, dispatch) {
    ...
    }, h(HTMLDListElement, {}, ...items.flatMap(([key, value]) => [
        h(HTMLElement, { localName: "dt" }, key),
        h(HTMLElement, { localName: "dd" }, value),
    ])), ...statusData.content, browserUiPositionSendKey === undefined
        ? undefined
        : viewBrowserUiPositionChooser(browserUiPosition, dispatch, browserUiPositionSendKey));
}
const allBrowserUiPositionsInOrder = [
    "TopLeft",
    "TopRight",
    "BottomLeft",
    "BottomRight",
    ...
    switch (status.tag) {
        ...
        case "EvalError":
            return {
                dl: [],
                content: [
                    h(HTMLParagraphElement, {}, "Check the console in the browser developer tools to see errors!"),
                ],
            };
        case "Idle":
            return {
        ...
        case "UnexpectedError":
            return {
                dl: [],
                content: [
                    h(HTMLParagraphElement, {}, "I ran into an unexpected error! This is the error message:"),
            ...
            return {
                ...
                content: model.elmCompiledTimestamp === model.elmCompiledTimestampBeforeReload
                    ? [
                        ...
                        `The old JavaScript was compiled ${new Date(model.elmCompiledTimestamp).toLocaleString()}, and so was the JavaScript currently running.`,
...
function viewOpenEditorError(error) {
    switch (error.tag) {
        case "EnvNotSet":
            return [
                h(HTMLDivElement, { className: CLASS.envNotSet }, h(HTMLParagraphElement, {}, "ℹ️ Clicking error locations only works if you set it up."), h(HTMLParagraphElement, {}, "Check this out: ", h(HTMLAnchorElement, {
        ...
        case "CommandFailed":
            return [
                h(HTMLParagraphElement, {}, h(HTMLElement, { localName: "strong" }, "Opening the location in your editor failed!")),
...
function viewHttpsInfo(webSocketUrl) {
    return webSocketUrl.protocol === "wss:"
        ? [
            h(HTMLParagraphElement, {}, h(HTMLElement, { localName: "strong" }, "Having trouble connecting?")),
            h(HTMLParagraphElement, {}, " You might need to ", h(HTMLAnchorElement, { href: new URL(`https://${webSocketUrl.host}/accept`).href }, "accept elm-watch’s self-signed certificate"), ". "),
            h(HTMLParagraphElement, {}, h(HTMLAnchorElement, {
...
function noDebuggerReason(noDebuggerProgramTypes) {
    return `The Elm debugger isn't supported by ${humanList(Array.from(noDebuggerProgramTypes, (programType) => `\`${programType}\``), "and")} programs.`;
}
...
function viewCompilationModeChooser({ dispatch, sendKey, compilationMode: selectedMode, warnAboutCompilationModeMismatch, info, }) {
    switch (info.initializedElmAppsStatus.tag) {
        case "DecodeError":
            return [
                h(HTMLParagraphElement, {}, "window.Elm does not look like expected! This is the error message:"),
                h(HTMLPreElement, {}, info.initializedElmAppsStatus.message),
            ];
        case "MissingWindowElm":
            return [
                h(HTMLParagraphElement, {}, "elm-watch requires ", h(HTMLAnchorElement, {
                    href: "https://lydell.github.io/elm-watch/window.Elm/",
                    target: "_blank",
                    rel: "noreferrer",
                }, "window.Elm"), " to exist, but it is undefined!"),
            ];
        case "NoProgramsAtAll":
            return [
                h(HTMLParagraphElement, {}, "It looks like no Elm apps were initialized by elm-watch. Check the console in the browser developer tools to see potential errors!"),
            ];
        case "DebuggerModeStatus": {
            ...
            return [
                h(HTMLFieldSetElement, { disabled: sendKey === undefined }, h(HTMLLegendElement, {}, "Compilation mode"), ...compilationModes.map(({ mode, name, status }) => {
                    const nameWithIcon = h(HTMLSpanElement, { className: CLASS.compilationModeWithIcon }, name, mode === selectedMode ? compilationModeIcon(mode) : undefined);
                    return h(HTMLLabelElement, { className: status.tag }, h(HTMLInputElement, {
                        type: "radio",
                        name: `CompilationMode-${info.targetName}`,
                        value: mode,
                        checked: mode === selectedMode,
                        disabled: sendKey === undefined || status.tag === "Disabled",
                        onchange: sendKey === undefined
                            ? undefined
                            : () => {
                                dispatch({
                                    tag: "ChangedCompilationMode",
                        ...
                        ? [
                            ...
                            warnAboutCompilationModeMismatch &&
                                ...
                                ? h(HTMLElement, { localName: "small" }, `Note: The code currently running is in ${ORIGINAL_COMPILATION_MODE} mode.`)
                                : undefined,
                        ]
                        : [
                            nameWithIcon,
...
function updateErrorOverlay(targetName, dispatch, sendKey, errors, overlay, overlayCloseButton) {
    const existingErrorElements = new Map(Array.from(overlay.children, (element) => [
        element.id,
        {
            targetNames: new Set(
    ...
    for (const [id, error] of errors) {
        ...
        if (maybeExisting === undefined) {
            const element = viewOverlayError(targetName, dispatch, sendKey, id, error);
            ...
            overlayCloseButton.style.setProperty("--foregroundColor", error.foregroundColor);
            overlayCloseButton.style.setProperty("--backgroundColor", error.backgroundColor);
            previousElement = element;
        }
        else {
            if (!maybeExisting.targetNames.has(targetName)) {
                maybeExisting.element.setAttribute(DATA_TARGET_NAMES, [...maybeExisting.targetNames, targetName].join("\n"));
            }
...
function viewOverlayError(targetName, dispatch, sendKey, id, error) {
    ...
    }, h(HTMLElement, { localName: "summary" }, h(HTMLSpanElement, {
        className: CLASS.errorTitle,
        style: {
            backgroundColor: error.backgroundColor,
        },
    }, error.title), error.location === undefined
        ? undefined
        : h(HTMLParagraphElement, {}, viewErrorLocation(dispatch, sendKey, error.location))), h(HTMLPreElement, { innerHTML: error.htmlContent }));
}
function viewErrorLocation(dispatch, sendKey, location) {
    switch (location.tag) {
        case "FileOnly":
            return viewErrorLocationButton(dispatch, sendKey, {
                file: location.file,
                line: 1,
                column: 1,
            }, location.file.absolutePath);
        case "FileWithLineAndColumn": {
            return viewErrorLocationButton(dispatch, sendKey, location, `${location.file.absolutePath}:${location.line}:${location.column}`);
        }
        case "Target":
...
function renderMockStatuses(getNow, elements) {
    if (elements === undefined) {
        return;
    }
    const date = getNow();
    ...
    const mockStatuses = {
        ...
        LongSubdomain: {
            ...
            info: {
                ...info,
                webSocketUrl: new URL("ws://development.admin.example.com.localhost:53167"),
            },
        },
        IPAdress: {
            tag: "Idle",
...
Maybe the JavaScript code running in the browser was compiled with an older version of elm-watch? If so, try reloading the page.
    ...
    for (const [targetName, status] of Object.entries(mockStatuses)) {
        ...
        render(getNow, targetRoot, () => {
            // Ignore messages.
        }, model, { ...(status.info ?? info), targetName }, false);
    }
}
