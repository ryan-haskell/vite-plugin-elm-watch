...
async function run(args) {
    ...
    if (!isNonEmptyArray(directories)) {
        throw new KnownError(`You must pass at least one directory to find Elm files in.`);
    }
    ...
    const elmFiles = directories.flatMap((dir) => findElmFiles(absolutePathFromString(cwd.path, dir)));
...
async function runStrategy(strategy, elmFiles) {
    switch (strategy) {
        case "readFileSync":
        ...
        case "createReadStreamForAwait":
            return (await Promise.all(elmFiles.map(createReadStreamForAwaitStrategy))).flat();
    }
}
function readFileSyncStrategy(elmFile) {
...
async function readFileStrategy(elmFile) {
...
function readSyncStrategy(elmFile) {
    const readState = Parser.initialReadState();
...
async function readStrategy(elmFile) {
    const readState = Parser.initialReadState();
    const fileHandle = await fsPromises.open(elmFile.absolutePath, "r");
    const buffer = Buffer.alloc(2048);
    let result;
    outer: while ((result = await fileHandle.read(buffer, 0, buffer.byteLength)).bytesRead > 0) {
...
async function createReadStreamStrategy(elmFile) {
...
async function createReadStreamForAwaitStrategy(elmFile) {
    const readState = Parser.initialReadState();
    const stream = fs.createReadStream(elmFile.absolutePath, {
        highWaterMark: 2048,
    });
