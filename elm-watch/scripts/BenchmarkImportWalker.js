...
import { isNonEmptyArray, mapNonEmptyArray, } from "../src/NonEmptyArray";
import { absoluteDirname, absolutePathFromString, findClosest, } from "../src/PathHelpers";
function run(args) {
    ...
    const inputPaths = mapNonEmptyArray(args, (elmFilePathRaw) => ({
        tag: "InputPath",
    ...
    const elmJsonPathsRaw = new HashSet(mapNonEmptyArray(inputPaths, (inputPath) => findClosest("elm.json", absoluteDirname(inputPath.theInputPath)) ?? {
        tag: "NotFound",
        inputPath,
    }));
    const uniqueElmJsonPathRaw = getSetSingleton(elmJsonPathsRaw);
    if (uniqueElmJsonPathRaw === undefined) {
        console.error("Could not find (a unique) elm.json for all of the input paths:", inputPaths);
        process.exit(1);
    }
    if (uniqueElmJsonPathRaw.tag === "NotFound") {
        console.error("Could not find elm.json for:", uniqueElmJsonPathRaw.inputPath);
        process.exit(1);
    }
    const elmJsonPath = {
        tag: "ElmJsonPath",
    ...
    let elmJson;
    try {
        elmJson = ElmJson(JSON.parse(fs.readFileSync(elmJsonPath.theElmJsonPath.absolutePath, "utf8")));
    }
    catch (error) {
        console.error(error instanceof Decode.DecoderError
            ? error.format()
    ...
    console.log("Elm file(s):", mapNonEmptyArray(inputPaths, (inputPath) => inputPath.theInputPath.absolutePath));
