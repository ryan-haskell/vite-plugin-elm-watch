...
async function run() {
    ...
    for (const { src, dest = src, transform } of FILES_TO_COPY) {
        if (transform !== undefined) {
            fs.writeFileSync(path.join(BUILD, dest), transform(fs.readFileSync(path.join(DIR, src), "utf8")));
        }
        else {
            fs.copyFileSync(path.join(DIR, src), path.join(BUILD, dest));
        }
    }
    fs.writeFileSync(path.join(BUILD, "package.json"), JSON.stringify({ ...PACKAGE_REAL, dependencies: PACKAGE.dependencies }, null, 2));
    fs.writeFileSync(path.join(BUILD, "ClientCode.js"), `
const fs = require("fs");
...
exports.proxy = fs.readFileSync(path.join(__dirname, "proxy.js"), "utf8");
    ...
    for (const output of clientResult.outputFiles) {
        switch (path.basename(output.path)) {
            case "client.js":
            case "proxy.js":
                fs.writeFileSync(output.path, output.text.replace(/%VERSION%/g, PACKAGE_REAL.version));
                break;
            default:
        ...
        switch (path.basename(output.path)) {
            case "index.js": {
                const replaced = output.text
                    .slice(secondIndexOf(output.text, "//"), output.text.lastIndexOf("//"))
