...
function checkFile(file) {
    ...
    try {
        ...
        console.info(`Skipping: ${relativeFile}:`, error instanceof Decode.DecoderError
    ...
    for (const [name, version] of Object.entries(allDependencies)) {
        ...
        if (version !== versionToInstall) {
            throw new Error(`This file includes \`${JSON.stringify(name)}: ${JSON.stringify(version)}\`, but the elm.json we install includes: \`${JSON.stringify(name)}: ${JSON.stringify(versionToInstall)}\``);
        }
    }
}
try {
