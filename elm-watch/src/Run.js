...
export async function run(cwd, env, logger, getNow, runMode, args, restartReasons, postprocessWorkerPool, webSocketState, hotKillManager) {
    const parseResult = ElmWatchJson.findReadAndParse(cwd);
    switch (parseResult.tag) {
        case "ReadAsJsonError":
            logger.errorTemplate(Errors.readElmWatchJsonAsJson(parseResult.elmWatchJsonPath, parseResult.error));
            return handleElmWatchJsonError(logger, getNow, runMode, parseResult.elmWatchJsonPath, postprocessWorkerPool);
        case "DecodeError":
            logger.errorTemplate(Errors.decodeElmWatchJson(parseResult.elmWatchJsonPath, parseResult.error));
            return handleElmWatchJsonError(logger, getNow, runMode, parseResult.elmWatchJsonPath, postprocessWorkerPool);
        ...
        case "Parsed": {
            ...
            switch (parseArgsResult.tag) {
                case "UnknownFlags":
                    logger.errorTemplate(Errors.unknownFlags(cwd, parseResult.elmWatchJsonPath, runMode, args, parseArgsResult.unknownFlags));
                    return { tag: "Exit", exitCode: 1 };
                ...
                case "Success": {
                    ...
                    const unknownTargetsSubstrings = parseArgsResult.targetsSubstrings.filter((substring) => !knownTargets.some((targetName) => targetName.includes(substring)));
                    if (isNonEmptyArray(unknownTargetsSubstrings)) {
                        logger.errorTemplate(Errors.unknownTargetsSubstrings(parseResult.elmWatchJsonPath, knownTargets, unknownTargetsSubstrings));
                        return { tag: "Exit", exitCode: 1 };
                    }
                    const elmWatchStuffDir = {
                        tag: "ElmWatchStuffDir",
                        theElmWatchStuffDir: absolutePathFromString(absoluteDirname(parseResult.elmWatchJsonPath.theElmWatchJsonPath), "elm-stuff", "elm-watch"),
                    };
                    const elmWatchStuffJsonPath = {
                        tag: "ElmWatchStuffJsonPath",
                        theElmWatchStuffJsonPath: absolutePathFromString(elmWatchStuffDir.theElmWatchStuffDir, "stuff.json"),
                    };
                    const elmWatchStuffJsonParseResult = runMode === "hot"
                    ...
                    switch (elmWatchStuffJsonParseResult?.tag) {
                        case "ElmWatchStuffJsonReadAsJsonError":
                            logger.errorTemplate(Errors.readElmWatchStuffJsonAsJson(elmWatchStuffJsonPath, elmWatchStuffJsonParseResult.error));
                            return { tag: "Exit", exitCode: 1 };
                        case "ElmWatchStuffJsonDecodeError":
                            logger.errorTemplate(Errors.decodeElmWatchStuffJson(elmWatchStuffJsonPath, elmWatchStuffJsonParseResult.error));
                            return { tag: "Exit", exitCode: 1 };
                        case undefined:
                        case "Parsed":
                        case "NoElmWatchStuffJson": {
                            const elmWatchStuffJson = elmWatchStuffJsonParseResult?.tag === "Parsed"
                            ...
                            const initProjectResult = initProject({
                                ...
                                enabledTargetsSubstrings: isNonEmptyArray(parseArgsResult.targetsSubstrings)
                            ...
                            switch (initProjectResult.tag) {
                                case "DuplicateOutputs":
                                    logger.errorTemplate(Errors.duplicateOutputs(parseResult.elmWatchJsonPath, initProjectResult.duplicates));
                                    return handleElmWatchJsonError(logger, getNow, runMode, parseResult.elmWatchJsonPath, postprocessWorkerPool);
                                // istanbul ignore next
                                case "NoCommonRoot":
                                    logger.errorTemplate(Errors.noCommonRoot(initProjectResult.paths));
                                    return { tag: "Exit", exitCode: 1 };
                                case "Project": {
                                    ...
                                    switch (project.postprocess.tag) {
                                        case "NoPostprocess":
                                            break;
                                        case "Postprocess":
                                            if (project.postprocess.postprocessArray[0] ===
                                                ELM_WATCH_NODE) {
                                    ...
                                    switch (runMode) {
                                        case "make": {
                                            const result = await Make.run(env, logger, getNow, project, postprocessWorkerPool);
                                            switch (result.tag) {
                                                case "Error":
                                                    return { tag: "Exit", exitCode: 1 };
                                                case "Success":
                                        ...
                                        case "hot": {
                                            const result = await Hot.run(env, logger, getNow, restartReasons, postprocessWorkerPool, webSocketState, project, config.port !== undefined
                                            ...
                                            switch (result.tag) {
                                                ...
                                                case "ExitOnIdle":
                                                    return {
                                                        tag: "Exit",
                                                        exitCode: result.reason === "CtrlCPressedOrStdinEnd"
                                                            ? 0
                                                            : 1,
                                                    };
                                                case "Restart":
...
async function handleElmWatchJsonError(logger, getNow, runMode, elmWatchJsonPath, postprocessWorkerPool) {
    switch (runMode) {
        ...
        case "hot": {
            ...
            const elmWatchJsonEvent = await Hot.watchElmWatchJsonOnce(getNow, elmWatchJsonPath);
            logger.clearScreen();
            return {
                tag: "Restart",
