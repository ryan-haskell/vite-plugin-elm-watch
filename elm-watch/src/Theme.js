...
const THEME_ESCAPES_STRING = Array.from({ length: 16 }, (_, i) => `\x1B]4;${i};?\x1B\\` // Palette.
)
...
const THEME_ESCAPES_REGEX = /\x1B](4;)?(\d+);rgb:([\da-f]{4})\/([\da-f]{4})\/([\da-f]{4})/gi;
...
export const COLOR_TO_TERMINAL_ESCAPE = {
    red: "\x1B[31m",
    RED: "\x1B[91m",
    magenta: "\x1B[35m",
    MAGENTA: "\x1B[95m",
...
export async function getThemeFromTerminal(logger) {
    ...
    const stdin = await logger.queryTerminal(THEME_ESCAPES_STRING, (stdinSoFar) => stdinSoFar.includes(THEME_ESCAPES_DONE_CHECK));
